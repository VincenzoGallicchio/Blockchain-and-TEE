...
class SimpleWalletTransactionHandler(TransactionHandler):
    ...
    def sendCommandToEnclave(self, command):
        ip = '192.168.1.67'
        port = 8989
        hostname='NAME'
        ...
        with create_connection((ip, port)) as client:
            with context.wrap_socket(client, server_hostname=hostname) as tls:
                ...
                data = tls.recv(1024).decode("utf-8") 
                LOGGER.info('RISPOSTA DEL SERVER')
                LOGGER.info(data)
                return data
    ...
    def apply(self, transaction, context):
        ...
        if operation == "deposit":
            self._make_deposit(context, amount, from_key, psw)
    ...
    def _make_deposit(self, context, amount, from_key, psw):
        wallet_address = self._get_wallet_address(from_key)
        if self.sendCommandToEnclave(psw) == 'SI':
            LOGGER.info(psw)
            ...
            command = "deposit " + amount + " " + from_key
