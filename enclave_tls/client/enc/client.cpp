...
int handle_communication_until_done(mbedtls_ssl_context* ssl)
{
    ...
    len = sprintf((char*)buf, "%s", (char*)pass);
...
int launch_tls_client(char* function, char* arg){
    int ret = 1;
    const char* pers = "ssl_client";
    oe_result_t result = OE_FAILURE;
    int exit_code = MBEDTLS_EXIT_FAILURE;
    char* server_name="localhost"; 
    char* server_port= arg;
	...
	if(strcmp((char*)function, (char*)pass) == 0)
		exit_code = MBEDTLS_EXIT_SUCCESS;
	else{ 
          exit_code = 50;
          goto exit;
    ...
    exit:
	    ...
	    printf("Chiamata tls dall'enclave terminata");
	    printf("%i", exit_code);
	    fflush(stdout);
